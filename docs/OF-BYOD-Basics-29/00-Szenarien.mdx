...
Wenn bei einem Szenario z.B. steht «an Ihre Lehrperson senden», dann senden Sie es beim Üben bitte **an eine Kolleg:in** oder an sich selber. Erst bei der Probe müssen Sie es an Ihre Lehrperson senden.
:::
...
### Szenario 12
- Teams öffnen und eine Nachricht mit einer kurzen Begrüssung, dem Text «Ich übe die BYOD-Szenarien.» und einem kurzen Gruss an Ihre Lehrperson schicken.

### Szenario 13
- Den Ordner `programme` komprimieren
- Die komprimierte Datei zu `programme_byod.zip` umbenennen
- Die komprimierte Datei per Mail an Ihre Lehrperson schicken (dabei einen passenden Betreff, eine Begrüssung und einen kurzen Gruss nicht vergessen!)
...
### Szenario 22
- Öffnen Sie Teams und starten Sie eine Video-Konferenz mit Ihrer Lehrperson
...
- Senden Sie den Link per Teams an Ihre Lehrperson
...
- Teams öffnen und als formatierten Python-Codeblock an Ihre Lehrperson senden (die Begrüssung und Grossformel darf ausnahmsweise weggelassen werden).
